...
class YoloDataSets(Dataset):
    ...
    def __init__(self, data_path, input_size=(1280, 768), batch_size=4, image_set='trainval', jitter_x=0.3,
                 jitter_y=0.3, augment=True, hue=0.1, saturation=1.5, exposure=1.5, angle=0, parameters=None):
        """
    ...
    def __getitem__(self, index):
        ...
        from data.prepare_data import PrepareDate
        prepare = PrepareDate()
        prepare.set_attribute(self.parameters)
        img = prepare(img, target)
        print(123)
        # for bbox in target:
        #     print(bbox)  # x y w h
        ...
        if self.augment:
            ...
            scale = 1
            com_ratio = self.input_size_width / self.input_size_height
            if new_ar < com_ratio:
...
if __name__ == '__main__':
    data_path = '~/datasets/sports-training-data/player_detection/training_dataset'
    data_set = YoloDataSets(data_path, image_set='train_freed_2k.txt')
    batch_size = 2
